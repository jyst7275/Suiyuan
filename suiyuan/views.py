...
def archives(request, year, month, day, title):
    ...
    url = request.path
    object_get = get_object_or_404(Passage, pass_title_index=title,
                                   pub_date__gte=datetime.datetime(int(year), int(month), int(day)))
    try:
...
def product_archives(request, cat):
    ...
    if cat == "all":
        ...
        product_list = Product.objects.filter(
            Q(product_category__father__category=cat) | Q(product_category__category=cat))
    if order_ba != False:
