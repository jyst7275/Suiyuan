...
class Passage(models.Model):
    ...
    pass_type = models.CharField('文章类型', max_length=20,
                                 choices=(("News", "公司新闻"), ("Business", "行业动态"), ("Health", "健康知识")))
    ...
    def get_absolute_url(self):
        return "/archives/{0}/{1}/{2}/{3}".format(self.pub_date.year, self.pub_date.month, self.pub_date.day,
                                                  self.pass_title_index) + '/'
    ...
    def save(self, force_insert=False, force_update=False, using=None, update_fields=None):
        if not self.pass_title_index:
            ...
            check_obg = Passage.objects.filter(
                pub_date__gte=datetime.datetime(self.pub_date.year, self.pub_date.month, self.pub_date.day),
                pass_title_index__startswith=title_index)
    ...
    def order_detail(self):
        ...
        for detail in detail_list:
            return_html += format_html('<li style="list-style:none;"><a href={}>{} ￥{} x{}</a></li>',
                                       '/admin/suiyuan/orderdetail/' + str(detail.id) + '/change/',
                                       detail.order_product.product_name, detail.order_price, detail.order_count)
