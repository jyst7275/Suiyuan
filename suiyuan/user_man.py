    ...
    if request.method == 'GET':
        if 'product' in request.GET:
            ...
            if request.GET['action'] == "post":
                try:
                    return_cart, total, total_count = cart_to_view(request.session['order_confirm']['cart_id_list'],
                                                                   request.session['order_confirm']['cart_count_list'])
...
def order_address(request):
    if request.method != 'POST':
        ...
        try:
            ...
            raise SuspiciousOperation
        address = Address.objects.create(name=name, province=province, city=city, country=country, detail=detail,
                                         cellphone=cellphone, user=request.user)
        address.save()
    return_json = {
...
def order_request(request):
    ...
    try:
        for i, ids in enumerate(id_list):
            ...
            order_detail = OrderDetail.objects.create(order_id=order, order_product=or_pr, order_count=count_list[i],
                                                      order_price=or_pr.product_prize)
...
def address_location(request):
    ...
    try:
        ...
        raise SuspiciousOperation
    url = 'http://api.map.baidu.com/geocoder/v2/?output=json&ak=u75trxs2Sm6TsanHVanYIopW&location={0},{1}'.format(lat,
                                                                                                                  long)
    r = requests.get(url)
    r_json = r.json()
    address_json = {
